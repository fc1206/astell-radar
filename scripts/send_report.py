...
Env: MAIL_USER (gmail/workspace address), MAIL_PASSWORD (app password),
     MAIL_TO (default frank@labtwofour.com), MAIL_SMTP (default smtp.gmail.com).
...
def main():
    ...
    to = os.environ.get("MAIL_TO", "frank@labtwofour.com")
    ...
    subject = f"Astell Radar {run_date}: +{len(new)} new"
    if new:
    ...
    msg.add_attachment(html.encode("utf-8"), maintype="text", subtype="html",
                       filename=f"astell-radar-{run_date}.html")
