...
TIERS = {"1", "2", "3"}
# The valid cluster set is MARKET-SPECIFIC and lives in config/clusters.json so the radar
# retargets by editing config, never this code. This is only the fallback when that file
# is absent (older checkouts / the test harness's throwaway repos).
...
# (conflating a YC batch label like F25 with the founding year; recording a US-based
# CEO's city as HQ when the company is elsewhere) and relocations are legitimate fixes
# the canonical writer should accept — the 2026-06-24 audit found ~47% of founding years
# and ~21% of HQs wrong, with no sanctioned path to correct them. Only identity fields
...
def load_clusters(root: Path) -> set:
    """Valid cluster set from config/clusters.json (so the radar retargets via config, not
