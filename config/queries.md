...
- 2026-06-25 (blocks H/I): Block I (edge-expansion / compare-list harvest) paid off exactly as designed — the Capterra/SourceForge "enterprise search alternatives" directory query surfaced **Atolio** (atolio.com, $24M, US Air Force/Cribl) and **Squirro** (squirro.com, ECB/Bank of England), two in-market direct-lane competitors with marquee logos that years of keyword + funding queries never reached because both sell on a *deployment-model* axis (private/in-VPC, regulated-finance) our vocabulary doesn't index. Confirms the Tanderrum thesis isn't only about non-US/no-funding invisibles — established, well-funded players hide in the wrong *positioning* axis too; read the competitor sets directories publish, don't re-guess vocabulary. Block H (Germany) added **Blockbrain** (€17.5M Series A, expertise-capture/knowledge-bots, Bosch/Roland Berger) — Interloom-adjacent T2. Status sweep low-yield again: AlphaSense's $350M/$7.5B was already in the registry `what` (only SuperAnalyst agent was new, no tier move); the one material catch was Databricks Genie GA + Managed MCP Server (primary). Lesson reconfirmed: before writing an AlphaSense-class status update, grep the registry row — the round was already captured. New digest-flag worth a standing wildcard: data incumbents exposing **MCP servers** over warehouse data (Genie GA) + Snowflake reportedly renaming its agent to "CoWork" — the data lane is grabbing both agent-access plumbing and work-AI naming.
