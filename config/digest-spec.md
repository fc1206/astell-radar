...
The digest answers one question: **given this run's findings, what should Lab24 do differently?** It is derived against `config/astell-context.md` and structurally enforced by `scripts/validate_digest.py` (the only writer of `data/DIGEST.md`).

## Process

1. After a successful merge, read: `config/astell-context.md`, this run's findings (candidates, status updates, scan notes), and the 3 most recent entries in `data/DIGEST.md` (novelty check).
...
**Signal:** <the fact, with evidence link or (registry-domain)>
**Why it matters to Astell:** <tie to a specific pillar, standing read, or actionable lane from the context file>
**Action:** <imperative, concrete, 2–8-week-shaped, names a deliverable or decision>
```

0–5 items. Zero findings → single line after the header: `NO ACTIONABLE SIGNAL — <one honest line on why this run's findings don't clear the bar>`. This outcome is respected; forced items are not.
...
- No re-issuing a prior digest's action without explicitly stating what NEW evidence changed.
...
Read each Action and ask: could Frank assign this to a person today, and would they know what done looks like? If no, cut or sharpen it.
