...
The founding failure (Tanderrum) was a RECALL miss — a real competitor in
...
- Block H  — geographic / non-US (the literal Tanderrum gap: Melbourne, no US press)
- Block I  — edge-expansion: harvest the competitor sets G2/rivals already publish
- Block B  — data-intelligence vocabulary (Tanderrum's own cluster)
...
check, documented in REARCHITECTURE_PLAN.md M2 — not a unit test. Green here does
...
def test_block_h_covers_non_us():
    ...
    assert any(r in body for r in regions), "Block H lost non-US geographic coverage (the Tanderrum gap)"
...
def test_block_b_has_data_intelligence_vocab():
    """Tanderrum's own cluster — wrong-vocabulary data-intelligence competitors."""
    body = BLOCKS["B"].lower()
    assert "data" in body and ("intelligence" in body or "analyst" in body or "bi" in body), \
        "Block B lost the data-intelligence vocabulary that catches the Tanderrum class"
...
def test_plan_run_sees_new_block(tmp_path):
    ...
    (root / "data/registry.csv").write_text(
        ...
        "glean.com,Glean,1,direct,active,series-f,US,2019,2026-06-10,2026-06-10,x,y,https://glean.com/,\n",
        encoding="utf-8",
    )
